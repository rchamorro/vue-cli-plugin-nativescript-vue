...
const isNativeOrDual = process.argv[3]
...
function Pre () {
    ...
    if(isNativeOrDual === 'web'){
      ...
      fs.copyFile('./node_modules/vue-cli-plugin-nativescript-vue/lib/scripts/dual.webpack.config.js', './webpack.config.js', (err) => {
        if (err) {
          console.error('copyFile Error occurred:', err);
          if (err) throw err;
        }
